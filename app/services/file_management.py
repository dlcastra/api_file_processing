...
class FileManagementService:
    ...
    async def add_file(self, file, user_id: int):
        ...
        if file_url["status"] == "success":
            new_file = FileModel(
                file_name=original_file_name, s3_url=file_url["file_url"], s3_key=s3_file_name, user_id=user_id
            )
    ...
    async def download_file(self, file_id: int, user_id: int):
        ...
        try:
            presigned_url = self.s3_client.generate_presigned_url(
                "get_object", Params={"Bucket": self.bucket, "Key": file.s3_key}, ExpiresIn=1800
            )
